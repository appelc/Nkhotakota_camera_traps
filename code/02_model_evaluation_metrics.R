...
## Plot precision/recall/F1 ----------------------------------------------------
  ...
  metrics_grid <- ggplot(metrics_filtered, aes(x = threshold, y = precision, color = 'Precision')) +
    ...
    rphylopic::geom_phylopic(aes(x = 0.6, y = 0.2, uuid = id, height = size_scaling), fill = 'black', color = NA) +
    geom_image(
      data = filter(phylopic_ids, !is.na(image_path)),
      aes(image = image_path, x = 0.6, y = 0.2),
      size = 0.25,
      color = 'black' # match visually to `size_scaling`
    ) +
