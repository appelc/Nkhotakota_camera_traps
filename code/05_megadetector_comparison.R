## Comparison of YOLO model with MegaDetector predictions
...
## Read in MegaDetector predictions ------------------------------
md_21 <- fread('Malawi_MegaDetector/megadetector_csvs_NWR2021/nwr_megadetector_2021.csv')
md_22 <- fread('Malawi_MegaDetector/megadetector_csvs_NWR2022/nwr_megadetector_2022.csv')
md_23 <- fread('Malawi_MegaDetector/megadetector_csvs_NWR2023/nwr_megadetector_2023.csv')
  
  #combine all years
  md_preds <- bind_rows(md_21, md_22, md_23)
  ...
  #empties are here now too (as of 7/14/24)
...
## Read in YOLO model predictions (validated only) -----------------------------
# preds_all <- fread('Malawi_evaluating/preds_all_073024.csv'); preds_all <- preds_all[,-'V1']
#   
#   #convert all classes to 'animal', 'human', or 'empty'
#   preds_all$class_simple <- ifelse(preds_all$species_cleaned == 'empty', 'empty',
#                                   ifelse(preds_all$species_cleaned == 'human', 'person',
#                                          'animal'))
#     table(preds_all$species_cleaned, useNA = 'a')
#     table(preds_all$class_simple, useNA = 'a') #make sure empty and person match above
#    
...
#   #summarize
#   nrow(preds_all)
#   length(unique(preds_all$filename)) #278,282 images (total validated were 282,557; the remaining were removed here bc of datetime errors I think?)
...
## Read in validated YOLO predictions ("true" classes) -------------------------
  ...
  #summarize
  length(unique(true_df$filename)) #278,282 images validated
  
   
## Read in YOLO model predictions (ALL images) ---------------------------------
preds_all <- preds_all <- fread('Malawi_processing/nwr_combined_preds_cleaned_061924.csv')

  #remove Chipata sites
  preds_all <- preds_all[preds_all$hex != 'CHI',] #remove Chipata sites
  head(preds_all); length(unique(preds_all$filename)) #3,489,218 images predicted with YOLO
...
## Cross-check -----------------------------------------------------------------
  ...
  length(unique(md_preds[!md_preds$filename_mod %in% unique(preds_all$filename),]$filename_mod))
    ...
    #No, 52,800 were not run through YOLO. Fix them later... remove for now. *******************************
  ...
  length(unique(preds_all[!preds_all$filename %in% unique(md_preds$filename_mod),]$filename))
    ...
    #No, 24,078 were not run thru MD. Fix them later... emove them for now. *******************************
    preds_all <- preds_all[preds_all$filename %in% unique(md_preds$filename_mod),]
    
  #What about true?
  nrow(true_df_sum[!true_df_sum$filename %in% unique(md_preds$filename_mod),]) #16 were validated but not run through MD; delete those here
  nrow(true_df_sum[!true_df_sum$filename %in% unique(preds_all$filename),]) #these same 16 
  
    #Remove those 16 (should get fixed when MD is re-run on R25-B-2022-11-03)
    true_df_sum <- true_df_sum[true_df_sum$filename %in% unique(md_preds$filename_mod),]
    
  nrow(preds_all[!preds_all$filename %in% true_df_sum$filename,]) #this is OK! 3,192,832 run thru YOLO but not validated
  nrow(md_preds[!md_preds$filename_mod %in% true_df_sum$filename,]) #this is OK! 3,841,244 run thru MD but not validated
...
## Iterate through thresholds --------------------------------------------------  
  ...
  (start <- Sys.time()) #takes about 10 minutes
  for (tt in thresh){
    ...
    #not quite perfect here... there are still multiple classes in some. had a hard time figuring out expanding to 1 row per.
    #we'll just ignore these later (e.g., "animal,human") bc there aren't that many.
    
      #inspect
      length(unique(all_tt_img[is.na(all_tt_img$true),]$filename)) #3,186,874 were not reviewed; that's OK
      length(unique(all_tt_img[is.na(all_tt_img$md),]$filename)) #good, I removed all the ones not run through MD
      length(unique(all_tt_img[is.na(all_tt_img$yolo),]$filename)) #good, I removed all the ones not run through YOLO
        # all_tt_img$site_check <- sapply(strsplit(all_tt_img$filename, '\\__'), '[', 1)
        # sort(unique(all_tt_img[is.na(all_tt_img$yolo),]$site_check))
        # table(all_tt_img[is.na(all_tt_img$yolo),]$site_check) #main culprits are D10-A-2021-12-03 (23k), D10-C-2021-12-03 (25k), G25-A-2023-07-27 (4k), K26-B-2023-08-10 (78)
  ...
  results_combined <- rbindlist(results_list)  
    ...
    tail(results_combined)
  
  #save
  saveRDS(results_list, 'Malawi_evaluating/results_081224/empty_results_list.RDS')
  write.csv(results_combined, 'Malawi_evaluating/results_081224/empty_results.csv')
...
## Create confusion matrices ---------------------------------------------------
  ...
  results_list <- readRDS('Malawi_evaluating/results_081224/empty_results_list.RDS')
  results_combined <- fread('Malawi_evaluating/results_081224/empty_results.csv')
    ...
    #Summary to report
    sum(cm_md_true$Freq) #279,487 predictions (not images, technically)
...
## Plot number of photos per threshold -----------------------------------------
  ...
  table(results_combined$thresh, results_combined$true, useNA = 'a') #318 NAs (that's OK, these weren't validated)
  ...
  #Plot
  # img_thresh <- ggplot(results_sum[results_sum$class %in% c('animal','empty') &
  #                                    results_sum$model %in% c('md','yolo'),], 
  #                      aes(x = thresh, y = n_images, color = model)) +
  #   geom_line(lwd = 1.5, aes(lty = model)) +
  #   facet_wrap(~class + validated, scales = 'free') +
  #   theme_bw()
  # img_thresh  
  
  img_thresh_both <- ggplot(results_sum[results_sum$class %in% c('animal') &
                                          results_sum$model %in% c('md','yolo') &
                                          results_sum$validated %in% c('all','y'),], 
                       aes(x = thresh, y = n_images, color = model)) +
    geom_line(lwd = 1.5, aes(lty = model)) +
    ylim(c(0,300000)) +
    facet_wrap(~validated) +
    theme_bw()
  img_thresh_both  
  
  img_thresh_all <- ggplot(results_sum[results_sum$class %in% c('animal') &
                                         results_sum$model %in% c('md','yolo') &
                                         results_sum$validated %in% c('all','y'),], 
                           aes(x = thresh, y = n_images, color = model)) +
    geom_line(aes(lty = validated), lwd = 1.2) +
    facet_wrap(~model) +
    theme_bw() + theme()
  img_thresh_all
  
  ggsave('figures/md_vs_yolo_new.png', img_thresh_all, dpi = 1000, width = 5, height = 5)
  
  
  
### To summarize and report ----------------------------------------------------
  cm_md_true
  sum(cm_md_true[cm_md_true$MegaDetector == 'animal',]$Freq) #124974 animals predicted by MD at 0.25
  
  results_sum 
  results_sum[results_sum$model == 'md' & results_sum$thresh == 0.25,] #209,023 animals predicted by MD at 0.25 (same on plot)
  results_sum[results_sum$model == 'yolo' & results_sum$thresh == 0.25,] #289,820 animals predicted by YOLO at 0.25 (same on plot)
  ...
  test_n_df <- data.frame(table(true_df$species_cleaned))
    ...
    #plot
    missed_df$md_found <- missed_df$test_n - missed_df$md_missed  
    missed_df$yolo_found <- missed_df$test_n - missed_df$yolo_missed  
    
    missed_long <- missed_df[,c(1:3,7:8)] %>% pivot_longer(cols = -class, names_to = c('method','category'),
                                                           names_sep = '_', values_to = 'number') %>%
      group_by(class, method)
    
    missed_plot <- ggplot(missed_long, aes(x = class, y = number, fill = category)) +
        geom_bar(stat = "identity", position = "fill") +
        facet_wrap(~ method, ncol = 2) +
        labs(x = "Class", y = "Proportion (%)", fill = "Category") +
        coord_flip() +
        geom_text(
          aes(label = number),   # Display percentage text
          position = position_fill(vjust = 0.5),            # Center text in bars
          color = "black", size = 3.5                       # Adjust color and size as needed
        ) +
        theme_bw() +
    ...
    #plot2
    missed_vs_total_md <- ggplot(missed_df, aes(x = md_found, y = prop_missed_md)) +
      geom_point(color = 'blue') +
      theme_bw()
    missed_vs_total_md
    
    missed_vs_total_yolo <- ggplot(missed_df, aes(x = yolo_found, y = prop_missed_yolo)) +
      geom_point(color = 'red') +
      theme_bw()
    missed_vs_total_yolo
...
## Calculate sensitivity/specificity -------------------------------------------    
  ...
  test_set_results$match_yolo_animal <- ifelse(grepl('animal', test_set_results$true), #does the image have an animal?
    ...
    #summarise
    length(unique(test_set_results$filename)) #278,266 images in test set
    length(unique(test_set_results[grepl('animal', test_set_results$true)]$filename)) #117,649 truly had animals
